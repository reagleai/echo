...
export default function Index() {
  ...
  const [workflowStatus, setWorkflowStatus] = useState<"success" | "failed" | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "execution_logs", filter: `user_id=eq.${user.id}` }, (payload: any) => {
        const newRecord = payload.new;
        if (activeLogIdRef.current && newRecord?.id === activeLogIdRef.current) {
          if (newRecord.status?.toLowerCase() === "success" || newRecord.status?.toLowerCase() === "completed") {
            clearPolling();
            setWorkflowStatus("success");
            setWorkflowError(null);
            toast({ title: "Workflow complete!", description: "Output ready — check your spreadsheet." });
          } else if (newRecord.status === "failed") {
            clearPolling();
            setWorkflowStatus("failed");
            setWorkflowError(newRecord.error_message || null);
            toast({ title: "Workflow failed", description: newRecord.error_message || "Something went wrong.", variant: "destructive" });
          }
        }
        fetchData();
      })
  ...
  useEffect(() => {
    if (!workflowActive || workflowStatus) {
      clearPolling();
      return;
    }
    ...
    pollingRef.current = setInterval(async () => {
      ...
      if (data.status?.toLowerCase() === "success" || data.status?.toLowerCase() === "completed") {
        ...
        clearPolling();
        setWorkflowStatus("failed");
        setWorkflowError(data.error_message || null);
        toast({ title: "Workflow failed", description: data.error_message || "Something went wrong.", variant: "destructive" });
        fetchData();
      }
    }, 2000);

    return clearPolling;
  }, [workflowActive, workflowStatus]);
    ...
    if (logsRes.data) {
      ...
      if (!workflowActive && !workflowStatus) {
        const runningLog = logsRes.data.find(
          (l: any) => l.status === "running" || l.status === "triggered"
        );
        if (runningLog) {
          activeLogIdRef.current = runningLog.id;
          const elapsedMs = Date.now() - new Date(runningLog.started_at).getTime();
          startTimeRef.current = Date.now() - elapsedMs;
          setInitialElapsed(elapsedMs);
  ...
  const triggerWorkflow = async () => {
    ...
    // Optimistically insert row exactly matching what the backend usually would
    ...
    try {
      ...
      // Update UI with explicit details returned synchronously from N8N webhook if no callback is used
      if (data && data.status) {
        ...
        clearPolling();
        setWorkflowStatus(data.status === "success" ? "success" : "failed");

        if (data.status === "error" || data.status === "failed") {
          setWorkflowError(data.error || "Unknown error occurred inside workflow.");
            ...
            <CardContent className="space-y-4">
              {!workflowActive && !workflowStatus ? (
                ...
                <WorkflowProgressBar
                  ...
                  completedStatus={workflowStatus}
